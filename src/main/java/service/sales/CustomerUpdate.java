...
public class CustomerUpdate implements Command {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		//유효한 접근인지 확인 
		if(request.getHeader("referer") == null || !request.getHeader("referer").contains("sales/customerUpdateForm.do")) {
			request.getSession().invalidate();
			request.setAttribute("result", -1);
			return "/view/sales/customerUpdateResult.jsp";
		}
		...
		int result = SalesDao.getInstance().updateCustomer(customer);
