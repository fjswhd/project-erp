...
public class CustomerInsert implements Command {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		//유효한 접근인지 확인 
		if(request.getHeader("referer") == null || !request.getHeader("referer").contains("sales/customerInsertForm.do")) {
			request.getSession().invalidate();
			request.setAttribute("result", -1);
			return "/view/sales/customerInsertResult.jsp";
		}
		...
		int result = SalesDao.getInstance().insertCustomer(customer);
