...
<link href="/project/css/purchase/sellerInsertForm.css?10" rel="stylesheet" type="text/css">
		...
		<div class="body">
			...
			<div class="content">
				<div class="content_head">
					<div class="label_name">구매처 수정</div>
				</div>
				<div class="content_body">
					<form action="/project/purchase/sellerUpdate.do" method="post" name="frm" onsubmit="return submitChk()">
					<table>
						<tr>
							<th>업체번호</th>
							<td>${seller.seller_no}<input type="hidden" name="seller_no" required="required" value="${seller.seller_no}" /></td>
						</tr>
						<tr>
							<th>업체명</th>
							<td><input type="text" name="seller_name" required="required" value="${seller.seller_name}" /></td>
						</tr>
						<tr>
							<th>사업자 번호</th>
							<td>
								<input type="text" name="seller_reg_num" required="required" value="${seller.seller_reg_num}" />
								<div class="msg"></div>
							</td>
						</tr>
						<tr>
							<th>전화번호</th>
							<td><input type="tel" name="seller_tel" required="required" value="${seller.seller_tel}" /></td>
						</tr>
						<tr>
							<th>이메일</th>
							<td><input type="email" name="seller_email" required="required" value="${seller.seller_email}" /></td>
						</tr>
						<tr class="addrRow">
							<th>주소</th>
							<td>
								<div>
									<input type="text" name="seller_addr_no" id="postcode" required="required" placeholder="우편번호" value="${seller.seller_addr_no}" />
									<input type="button" value="우편번호 입력" onclick="DaumPostcode()">
								</div>
								<div><input type="text" name="seller_addr" id="address" required="required" placeholder="주소" value="${seller.seller_addr}" /></div>
								<div><input type="text" name="seller_addr_detail" id="detailAddress" required="required" placeholder="상세주소" value="${seller.seller_addr_detail}" /></div>
							</td>
						</tr>
						<tr>
							<th>담당자</th>
						...
						<tr class="last">
							<th>참고사항</th>
							<td>
								<textarea name="seller_memo" rows="2" cols="40">${seller.seller_memo}</textarea>
							</td>
						</tr>
						<tr>
