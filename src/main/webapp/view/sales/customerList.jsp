				...
				<div class="content_body">
						...
						<c:if test="${not empty customerList }">
							<c:forEach var="customer" items="${customerList }">
								<tr>
									...
									<td>
										<a href="/project/sales/customerUpdateForm.do?customer_no=${customer.customer_no}"> ${customer.customer_no}</a>
									</td>
					...
					<div class="page">
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<c:if test="${param.p == i}">
								<b><a href="/project/sales/customerList.do?p=${i}">${i}</a></b>
							</c:if>
							<c:if test="${param.p != i}">
								<a href="/project/sales/customerList.do?p=${i}">${i}</a>
							</c:if>
						</c:forEach>
						<!-- 	보여줄 것이 아직 남아있다 -->
